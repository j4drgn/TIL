...
-- 도서를 구매한 적이 있는 고객의 고객명, 고객 주소 조회
-- 1. BOOKSALE의 고객정보(BOOKNO)를 조회
-- 2. CLIENT 테이블에서 조회된 CLIENTNO에 해당하는 고객 레코드만 추출한 후에 필요 컬럼 추출
SELECT CLIENTNAME, CLIENTADDRESS
FROM CLIENT
WHERE CLIENTNO IN (SELECT CLIENTNO
                   FROM BOOKSALE);

-- 도서를 한번도 구매한 적이 없는 고객의 번호와 고객명 조회
SELECT CLIENTNAME, CLIENTADDRESS
FROM CLIENT
...
WHERE CLIENTNO in (SELECT CLIENTNO
                   ...
                   WHERE BOOKNO in (SELECT BOOKNO
                                    FROM BOOK
                                    WHERE BOOKNAME = '안드로이드 프로그래밍'));
-- 서브쿼리를 사용했더라도 서브쿼리의 결과는 조건 값이므로
...
-- NULL 값을 포함하고 있는 컬럼 차이점 : IN VS EXISTS
...
SELECT CLIENTNO
FROM CLIENT
WHERE EXISTS (SELECT CLIENTHOBBY FROM CLIENT);

-- IN : 서브쿼리 결과에 NULL 값이 포함되지 않음.
-- 취미 정보가
SELECT CLIENTNO
FROM CLIENT
WHERE CLIENTHOBBY IN (SELECT CLIENTHOBBY FROM CLIENT);

-----------------------------------------------------------------
-- ALL/ANY
-- 관계연산자와 같이 사용
-- ALL : 서브쿼리의 결과 리스트가 조건검사대상이 되는 컬럼의 값과 비교할 때
--       리스트의 모든 값이 비교 연산에 대해 참이 되는 경우 참
-- 컬럼값이 10 > ALL(3,4,5,6) -> 참
-- ANY : 서브쿼리의 결과 리스트가 조건검사대상이 되는 컬럼의 값과 비교할 때
--       리스트의 어떤 한 값이라도 비교 연산에 대해 참이 되는 경우 참
-- 컬럼값이 10 > ALL(11,12,5,13) -> 참

-- 2번 고객이 주문한 도서의 최고 주문 수량보다 더 많은 도서를 구입한 고객의 고객 번호, 주문번호, 주문 수량 출력
...
-- 2번 고객의 주문 내역들과 비교할 때 한번이라도 더 많은 주문을 한 적이 있는 고객(2번 고객이 3번 주문한 주문 수량들 중 어떤 수량이어도 더 많이 주문한 고객의 고객번호)
-- 주문 수량을 조회
-- 2번 고객의 최소 주문 수량보다 많이 주문했던 고객의 주문 정보

-- 2번 고객을 포함한 모든 고객에 대해 2번 고객이 주문한 수량중 최소 주문 수량보다 많이 주문한 주문 정보
SELECT CLIENTNO, BSNO, BSQTY
FROM BOOKSALE
...
-- 2번 고객이 주문한 최소 주문  수량보다 많이 주문한 고객의 주문 정보(2번 고객은 제외)
SELECT CLIENTNO, BSNO, BSQTY
FROM BOOKSALE
WHERE BSQTY > ANY (SELECT BSQTY
                   FROM BOOKSALE
