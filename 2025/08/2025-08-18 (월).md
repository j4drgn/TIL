# 2025-08-18 (월)

## Java 프로젝트 - JavaProjectEx

JavaProjectEx는 Java와 JDBC를 활용한 도서 관리 프로그램으로, MVC 패턴을 적용하여 구현한 콘솔 애플리케이션입니다. 이 프로젝트는 도서 정보의 등록, 조회, 수정, 삭제 기능을 제공하며, 데이터베이스와의 연동을 통해 정보를 영구적으로 저장합니다.

### 프로젝트 구조

프로젝트는 MVC(Model-View-Controller) 아키텍처 패턴을 따르며 각 계층이 명확하게 분리되어 있습니다. 전체 구조는 다음과 같습니다:

```
src/
├── controller/
│   └── Controller.java
├── model/
│   ├── BookDAO.java
│   ├── BookDTO.java
│   └── IBookDAO.java
├── util/
│   └── DBConnect.java
└── view/
    ├── BookDeleteView.java
    ├── BookInsertView.java
    ├── BookListView.java
    ├── BookUpdateView.java
    ├── MainView.java
    └── ResultView.java
```

### MVC 패턴 분석

#### Model
- **BookDTO**: 도서 정보를 담는 Data Transfer Object로, 도서번호, 도서명, 저자, 가격, 발행일, 재고, 출판사번호 등의 속성을 가짐
- **IBookDAO**: 데이터 접근 인터페이스로, CRUD 기능을 정의
- **BookDAO**: IBookDAO 인터페이스를 구현하여 실제 데이터베이스 작업 수행

#### View
- **MainView**: 메인 메뉴를 표시하고 사용자 입력을 처리
- **BookInsertView**: 도서 정보 등록 화면
- **BookListView**: 도서 정보 조회 화면
- **BookUpdateView**: 도서 정보 수정 화면
- **BookDeleteView**: 도서 정보 삭제 화면
- **ResultView**: 작업 결과 메시지 표시

#### Controller
- **Controller**: View와 Model 사이의 중재자 역할, 싱글톤 패턴으로 구현되어 단일 인스턴스만 생성

### 주요 기능

1. **도서 정보 등록**: 도서번호, 제목, 가격, 발행일, 출판사번호 입력 후 DB에 저장
2. **도서 정보 조회**: DB에서 모든 도서 정보를 조회하여 표시
3. **도서 정보 수정**: 도서번호로 기존 도서 정보를 수정
4. **도서 정보 삭제**: 도서번호로 도서 정보를 삭제

### 프로그램 실행 흐름

아래 다이어그램은 프로그램의 전체적인 실행 흐름을 보여줍니다:

```mermaid
graph TD
    A[사용자] --> B[MainView]
    B --> C{메뉴 선택}
    C -->|1. 도서정보 등록| D[BookInsertView]
    C -->|2. 도서정보 조회| E[BookListView]
    C -->|3. 도서정보 수정| F[BookUpdateView]
    C -->|4. 도서정보 삭제| G[BookDeleteView]
    C -->|5. 종료| H[프로그램 종료]
    
    D --> I[Controller.insert]
    E --> J[Controller.getAllBook]
    F --> K[Controller.update]
    G --> L[Controller.delete]
    
    I --> M[BookDAO.insert]
    J --> N[BookDAO.getAllBook]
    K --> O[BookDAO.update]
    L --> P[BookDAO.delete]
    
    M --> Q[DBConnect]
    N --> Q
    O --> Q
    P --> Q
    
    Q --> R[(Oracle DB)]
    
    M --> S[ResultView]
    N --> T[BookListView.showAllBook]
    O --> S
    P --> S
```

### 클래스 다이어그램

아래 다이어그램은 프로젝트의 클래스 구조와 관계를 보여줍니다:

```mermaid
classDiagram
    class BookDTO {
        -String bookNo
        -String bookName
        -String bookAuthor
        -int bookPrice
        -String bookDate
        -int bookStock
        -String pubNo
        +getters()
        +setters()
        +toString()
    }
    
    class IBookDAO {
        <<interface>>
        +getAllBook() Vector~BookDTO~
        +insert(BookDTO) boolean
        +update(BookDTO) boolean
        +delete(BookDTO) boolean
    }
    
    class BookDAO {
        +getAllBook() Vector~BookDTO~
        +insert(BookDTO) boolean
        +update(BookDTO) boolean
        +delete(BookDTO) boolean
    }
    
    class DBConnect {
        +getConnection() Connection
        +close(Connection, PreparedStatement, ResultSet) void
        +close(Connection, PreparedStatement) void
        +close(Connection) void
        +close(PreparedStatement, ResultSet) void
        +close(PreparedStatement) void
    }
    
    class Controller {
        -static Controller instance
        -Controller()
        +getInstance() Controller
        +insert(BookDTO) void
        +getAllBook() void
        +update(BookDTO) void
        +delete(String) void
    }
    
    class MainView {
        +main(String[]) void
    }
    
    class BookInsertView {
        -Controller controller
        -Scanner sc
        +insert() void
    }
    
    class BookListView {
        -Controller controller
        +getAllBook() void
        +showAllBook(Vector~BookDTO~) void
    }
    
    class BookUpdateView {
        -Controller controller
        -Scanner sc
        +updateBook() void
    }
    
    class BookDeleteView {
        -Controller controller
        -Scanner sc
        +delete() void
    }
    
    class ResultView {
        +succMsg(String) void
        +errorMsg(String) void
    }
    
    IBookDAO <|.. BookDAO
    BookDAO --> DBConnect
    BookDAO --> BookDTO
    Controller --> BookDAO
    Controller --> ResultView
    Controller --> BookListView
    BookInsertView --> Controller
    BookListView --> Controller
    BookUpdateView --> Controller
    BookDeleteView --> Controller
    MainView --> BookInsertView
    MainView --> BookListView
    MainView --> BookUpdateView
    MainView --> BookDeleteView
```

### 구현 예제

#### 1. 싱글톤 패턴 구현 (Controller.java)
```java
public class Controller {
  // 본인 객체 인스턴스를 본인이 제공 - private static으로 객체 구성
  private static Controller instance = new Controller();

  private Controller() {
    //new 연산자로 생성자 호출 불가능하게 막음
  }

  //외부호출가능 클래스명으로 바로 호출 가능
  public static Controller getInstance() {
    return instance; //호출되면 공유 instance의 참조값을 반환
  }
}
```

#### 2. DAO 인터페이스 정의 (IBookDAO.java)
```java
public interface IBookDAO {
  public Vector<BookDTO> getAllBook() throws Exception; //전체 도서 정보 조회
  public boolean insert(BookDTO dto) throws Exception;
  public boolean update(BookDTO dto) throws Exception;
  public boolean delete(BookDTO dto) throws Exception;
}
```

#### 3. 데이터베이스 연결 및 자원 관리 (DBConnect.java)
```java
public static Connection getConnection() {
  Connection con = null;
  
  try {
    String url = "jdbc:oracle:thin:@localhost:1521:xe";
    String user = "SQL_USER"; //book table 사용
    String pwd = "1234";
    
    con = DriverManager.getConnection(url,user,pwd);
    
    if(con != null) {
      System.out.println("DB연결 성공!");
    }else {
      System.out.println("DB연결 실패!");
    }
  }catch(Exception e) {
    e.printStackTrace();
  }
  return con;    
}
```

#### 4. 예외 처리 및 전파 (BookDAO.java)
```java
@Override
public boolean insert(BookDTO dto) throws Exception {
  //한권 도서 정보 저장
  Connection con = null;
  PreparedStatement pstmt = null;
  try {
    con = DBConnect.getConnection();
    pstmt = con.prepareStatement(
        "insert into book values(?, ?, ?, ?, ?)"
    );
    pstmt.setString(1, dto.getBookNo());
    pstmt.setString(2, dto.getBookName());
    pstmt.setInt(3, dto.getBookPrice());
    pstmt.setString(4, dto.getBookDate());
    pstmt.setString(5, dto.getPubNo());

    int result = pstmt.executeUpdate();
    if (result == 0) {
      return false;
    }
  } catch (Exception e) {
    throw e; //호출한 쪽에서 예외처리하도록 예외를 인위적으로 발생 전달
  } finally {
    DBConnect.close(con);
  }
  return true;
}
```

### 학습 포인트

1. **MVC 패턴**: 사용자 인터페이스와 비즈니스 로직, 데이터 접근 계층을 분리하여 유지보수성 향상
2. **싱글톤 패턴**: 단일 인스턴스만 생성하여 자원 관리 효율화
3. **DAO/DTO 패턴**: 데이터 접근 로직과 데이터 전송 객체를 분리하여 코드 재사용성 향상
4. **인터페이스 기반 설계**: 구현체와 인터페이스 분리로 유연성 확보
5. **JDBC 활용**: 데이터베이스 연결 및 쿼리 실행, 자원 관리
6. **예외처리**: 계층별 예외 전파 및 처리 방식
7. **자원 관리**: try-catch-finally 블록을 통한 안전한 자원 반환

## Java 프로젝트 - game_project

game_project는 Java와 JDBC를 활용한 간단한 게임 애플리케이션으로, 사용자 관리 기능과 여러 미니게임을 제공하는 콘솔 기반 프로그램입니다.

### 프로젝트 구조

프로젝트는 패키지 기반으로 기능별로 명확하게 분리되어 있으며, 전체 구조는 다음과 같습니다:

```
src/
├── db/
│   └── DBConnection.java
├── exception/
│   ├── GameException.java
│   ├── InputValidationException.java
│   ├── SystemException.java
│   └── UserException.java
├── game/
│   ├── GaBaBo.java
│   ├── Game.java
│   ├── GameFactory.java
│   ├── GameInterface.java
│   ├── GameUtils.java
│   └── Guess.java
├── info/
│   └── AppInfo.java
├── start/
│   ├── AppStart.java
│   ├── MenuManager.java
│   └── UserManager.java
├── user/
│   ├── UserDAO.java
│   ├── UserDAOImpl.java
│   ├── UserDTO.java
│   └── UserSession.java
└── user.sql
```

### 주요 패키지 및 클래스 분석

#### 1. start 패키지
- **AppStart**: 애플리케이션의 진입점으로, 메인 로직과 전체 프로그램 흐름 관리
- **MenuManager**: 메뉴 표시 및 사용자 입력 처리 담당
- **UserManager**: 사용자 관련 기능(로그인, 회원가입, 정보 수정 등) 처리

#### 2. user 패키지
- **UserDTO**: 사용자 정보를 캡슐화한 데이터 전송 객체
- **UserDAO**: 사용자 데이터 접근 인터페이스 정의
- **UserDAOImpl**: UserDAO 인터페이스 구현체, 싱글톤 패턴 적용
- **UserSession**: 현재 로그인된 사용자 정보 관리, 싱글톤 패턴 적용

#### 3. game 패키지
- **GameInterface**: 모든 게임이 구현해야 하는 인터페이스
- **Game**: 게임의 공통 기능을 구현한 추상 클래스
- **GaBaBo**: 가위바위보 게임 구현
- **Guess**: 숫자 맞추기 게임 구현
- **GameFactory**: 게임 객체 생성을 담당하는 팩토리 클래스, 팩토리 패턴 적용

#### 4. db 패키지
- **DBConnection**: 데이터베이스 연결 관리, 싱글톤 패턴 적용

#### 5. exception 패키지
- **GameException**: 게임 관련 예외 클래스
- **UserException**: 사용자 관련 예외 클래스
- **SystemException**: 시스템 관련 예외 클래스
- **InputValidationException**: 입력 유효성 검증 관련 예외 클래스

### 클래스 다이어그램

아래 다이어그램은 프로젝트의 주요 클래스 구조와 관계를 보여줍니다:

```mermaid
classDiagram
    class AppStart {
        -UserDAO userDAO
        -UserSession userSession
        +main(String[] args)
        -initializeSystem()
        -processLoginMenuChoice(Scanner, int)
        -processMainMenuChoice(Scanner, int, AppInfo)
        -handleLogout()
        -playGame(Scanner, GameType)
    }
    
    class UserDTO {
        -String id
        -String password
        -String name
        +getters()
        +setters()
    }
    
    class UserDAO {
        <<interface>>
        +saveUser(UserDTO) boolean
        +findUserById(String) UserDTO
        +login(String, String) UserDTO
        +getAllUsers() List~UserDTO~
        +updateUser(UserDTO) boolean
        +withdrawUser(String) boolean
    }
    
    class UserDAOImpl {
        -static UserDAOImpl instance
        -UserDAOImpl()
        +getInstance() UserDAOImpl
        +saveUser(UserDTO) boolean
        +findUserById(String) UserDTO
        +login(String, String) UserDTO
        +getAllUsers() List~UserDTO~
        +updateUser(UserDTO) boolean
        +withdrawUser(String) boolean
    }
    
    class UserSession {
        -static UserSession instance
        -UserDTO currentUser
        -UserSession()
        +getInstance() UserSession
        +login(UserDTO) void
        +logout() void
        +isLoggedIn() boolean
        +getCurrentUser() UserDTO
    }
    
    class DBConnection {
        -static DBConnection instance
        -DBConnection()
        +getInstance() DBConnection
        +getConnection() Connection
        +closeResources(Connection, AutoCloseable, AutoCloseable) void
    }
    
    class GameInterface {
        <<interface>>
        +getGameName() String
        +getGameDescription() String
        +play(Scanner) void
    }
    
    class GameFactory {
        +enum GameType
        +createGame(GameType) GameInterface
        +createGameByName(String) GameInterface
    }
    
    class Game {
        <<abstract>>
        #String gameName
        #String description
        +getGameName() String
        +getGameDescription() String
        +play(Scanner) void
    }
    
    class GaBaBo {
        +play(Scanner) void
    }
    
    class Guess {
        +play(Scanner) void
    }
    
    class MenuManager {
        +displayLoginMenu(Scanner) int
        +displayMainMenu(Scanner, UserDTO) int
        +clearInputBuffer(Scanner) void
        +waitForKeyPress(Scanner) void
    }
    
    class UserManager {
        +login(Scanner, UserDAO, UserSession) void
        +register(Scanner, UserDAO) void
        +updateUserInfo(Scanner, UserDAO, UserSession) void
        +withdrawUser(Scanner, UserDAO, UserSession) boolean
    }
    
    AppStart --> UserDAO
    AppStart --> UserSession
    AppStart --> MenuManager
    AppStart --> UserManager
    AppStart --> GameFactory
    UserDAO <|.. UserDAOImpl
    UserDAOImpl --> DBConnection
    UserDAOImpl --> UserDTO
    UserSession --> UserDTO
    GameInterface <|.. Game
    Game <|-- GaBaBo
    Game <|-- Guess
    GameFactory ..> GaBaBo
    GameFactory ..> Guess
    UserManager --> UserDAO
    UserManager --> UserSession
```

### 주요 디자인 패턴

#### 1. 싱글톤 패턴
여러 클래스에서 싱글톤 패턴이 적용되어 있습니다:
- **UserDAOImpl**: 데이터베이스 접근 객체를 단일 인스턴스로 관리
- **UserSession**: 사용자 세션 정보를 애플리케이션 전체에서 공유
- **DBConnection**: 데이터베이스 연결을 중앙에서 관리

```java
// DBConnection.java의 싱글톤 패턴 구현
public class DBConnection {
    private static DBConnection instance;
    
    private DBConnection() {
        try {
            Class.forName("oracle.jdbc.driver.OracleDriver");
        } catch (ClassNotFoundException e) {
            System.out.println("Oracle JDBC 드라이버를 찾을 수 없습니다: " + e.getMessage());
            e.printStackTrace();
        }
    }
    
    public static DBConnection getInstance() {
        if (instance == null) {
            instance = new DBConnection();
        }
        return instance;
    }
}
```

#### 2. 팩토리 패턴
**GameFactory** 클래스는 팩토리 패턴을 구현하여 게임 객체 생성을 캡슐화합니다:

```java
public class GameFactory {
    public enum GameType {
        GABABO,
        GUESS
    }
    
    public static GameInterface createGame(GameType type) {
        switch (type) {
            case GABABO:
                return new GaBaBo();
            case GUESS:
                return new Guess();
            default:
                throw new IllegalArgumentException("지원하지 않는 게임 타입입니다: " + type);
        }
    }
}
```

#### 3. DAO 패턴
**UserDAO** 인터페이스와 **UserDAOImpl** 구현체를 통해 데이터 접근 로직을 캡슐화합니다:

```java
public interface UserDAO {
    boolean saveUser(UserDTO user) throws UserException;
    UserDTO findUserById(String id);
    UserDTO login(String id, String password) throws UserException;
    List<UserDTO> getAllUsers();
    boolean updateUser(UserDTO user) throws UserException;
    boolean withdrawUser(String id) throws UserException;
}
```

#### 4. 계층화된 예외 처리
프로젝트는 다양한 예외 클래스를 통해 체계적인 예외 처리를 구현합니다:
- **GameException**: 게임 실행 중 발생하는 예외
- **UserException**: 사용자 관련 작업 중 발생하는 예외
- **SystemException**: 시스템 관련 오류

### 학습 포인트

1. **패키지 기반 구조화**: 기능별로 패키지를 분리하여 코드 구조화
2. **디자인 패턴 적용**: 싱글톤, 팩토리, DAO 패턴 등 다양한 디자인 패턴 활용
3. **인터페이스 기반 설계**: 인터페이스를 통한 추상화로 코드 유연성 확보
4. **계층화된 예외 처리**: 예외 유형별 클래스 정의로 체계적인 오류 관리
5. **데이터베이스 연동**: JDBC를 활용한 사용자 정보 저장 및 관리
6. **세션 관리**: 로그인 상태 유지 및 권한 관리
7. **메뉴 기반 사용자 인터페이스**: 콘솔 기반 메뉴 시스템 구현